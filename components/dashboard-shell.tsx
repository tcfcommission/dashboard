...
function IntegrationsView({ data, onAdd, sync, mutate, remove, busy }: { data: DashboardData; onAdd: () => void; sync: (i: Integration) => void; mutate: (r: string, id: string, b: Record<string, unknown>) => void; remove: (r: string, id: string) => void; busy: string | null }) {
  return <PageSection eyebrow="Automation layer" title="Connect once. Let the system repeat." text="Secrets stay in your secure hosting environment. This dashboard stores only account IDs, safe URLs and environment-variable references." action={<button className="primary-button" onClick={onAdd}><Plus size={17}/> Add integration</button>}>
